...
def get_H(save_directory, images_list, world_points):
    ...
    for i, image in enumerate(copy_images):
        ...
        ret, corners_list = cv2.findChessboardCorners(grayscale, (7, 7), None)
        if ret: 
            print(f"{i}: Checkerboard {'encontrado' if ret else 'no encontrado'}")
            ...
            cv2.drawChessboardCorners(image, (7, 7), corners, True)
...
def get_K(H_total):
    ...
    if V.shape[0] == 0:
        raise ValueError("La matriz V está vacía. Verifica que se hayan generado homografías correctamente.")
    return K
...
def get_loss_function(K ,corner_list, world_coordinates, R):
    ...
    for i, corner_list in enumerate(corner_list):
        ...
        for j in range(len(world_coordinates)):
            ...
            proj_pts /= proj_pts[2] 
            x, y = (proj_pts[0]).item(), (proj_pts[1]).item()

            projected_coordinates = (K_ @ M)
            # print("size of H", H.shape)
            # print("size of H", world_points.shape)
            u = (projected_coordinates[0] / projected_coordinates[2]).item()
            v = (projected_coordinates[1] / projected_coordinates[2]).item()
            ...
            # r = x**2 + y**2
            # print(x)
            u_hat = (u + (u-uo)*(k1*(x**2 + y**2) + k2*((x**2 + y**2)**2))).item()
            # print("u_hat", u_hat)
            v_hat = (v + (v-vo)*(k1*(x**2 + y**2 )+ k2*((x**2 + y**2)**2))).item()

            try:
                corrected_corner = np.array([u_hat, v_hat, 1], dtype=np.float32)
            except ValueError as e:
                print(f"Advertencia: Se detectó un error en corrected_corner - {e}")
                corrected_corner = np.array([0, 0, 1], dtype=np.float32)  # Valor por defecto

            error = np.linalg.norm(corner_points - corrected_corner, 2)
            # print ("error", error)
            total_error = total_error + error
        
        f_error.append(total_error/49)
...
def get_error(updated_K ,updated_K_distorted, corner_list, world_coordinates, R):
    ...
    for i,corner in enumerate(corner_list):
        ...
        for j in range(world_coordinates.shape[0]):
            ...
            x = (image_coordinates[0] / image_coordinates[2]).item()
            y = (image_coordinates[1] / image_coordinates[2]).item()
    ...
    print("Media de error por reproyeccion: ", re_error_avg)
...
def main():
    ...
    Images = [cv2.imread(file) for file in glob.glob(image_path + '/*.jpeg')]# reading image from the image directory
    world_coordinates_x, world_coordinates_y = np.meshgrid(range(7), range(7))# the actual coordinates will be multiple of this size of box

    world_coordinates_x =world_coordinates_x.reshape(49, 1)# reshaping the coordinates
    world_coordinates_y =world_coordinates_y.reshape(49, 1)# reshaping the coordinates
    world_coordinates = np.array(np.hstack((world_coordinates_x, world_coordinates_y)).astype(np.float32)*24)# stacking all values converting to float and multiplying with actual value to get world coordinates
    ...
    if len(H_total) == 0:
        raise ValueError("No se encontraron homografías. Asegúrate de tener imágenes válidas de tablero de ajedrez en 'pruebas/*.jpeg'")
    K = get_K(H_total)
    ...
    print("K matrix: \n", K_final)
    distortion = np.array([
    (distortion_updated[0, 0]).item(),
    (distortion_updated[1, 0]).item(),
    0.0,
    0.0
], dtype=np.float32)

    print('Distortion optimized: ', distortion_updated.ravel())
