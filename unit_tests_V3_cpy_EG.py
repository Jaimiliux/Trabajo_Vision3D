...
def normalizar_puntos_(puntos):
    # Calcular el centroide
    centroide = np.mean(puntos, axis=0)
    # Desviacion estandar
    std = np.std(puntos, axis=0)
    #Escalar para que la distancia media sea raiz de 2
    escala = np.sqrt(2) / std
    # Matriz de transformación
    T = np.array([
        [escala[0], 0, -escala[0] * centroide[0]],
        [0, escala[1], -escala[1] * centroide[1]],
        [0, 0, 1]
    ])
...
def ransac(puntos_clave_l, puntos_clave_d, iter, t):
    ...
    for _ in range(iter):
        ...
        sample_l_, T1= normalizar_puntos_(sample_l_)
        sample_d_, T2 = normalizar_puntos_(sample_d_)
        ...
        '''
        for i, (pl_n, pd_n) in enumerate(zip(puntos_clave_l, puntos_clave_d)):
        ...
        '''
        #Error geometrico
        error = error_geometrico(puntos_clave_l, puntos_clave_d, F)
        #if error < t:
        ...
        if len(inliers_idx) > len(best_inliers):
            ...
            print("inliers hasta el momento = ", len(best_inliers))
    ...
    #    best_points_l, T_1 = normalizar_puntos_(puntos_clave_l[best_inliers])
    #    best_points_d, T_2 = normalizar_puntos_(puntos_clave_d[best_inliers])
    #    best_F = eight_point_algorithm(best_points_l, best_points_d, T_1, T_2)
    
    print(f"Total inliers found: {len(best_inliers)} (threshold t={t})")
...
def calcular_matriz_E(F,K,T1,T2):
    K_np = np.array(K)
    K_trans = K_np.T
    E = K_trans @ F @ K_np
    U, S, Vt = np.linalg.svd(E)
    S[-1] = 0  # Forzar rango 2
    E = U @ np.diag(S) @ Vt
    
    return E

def mostrar_linea_epipolar(E, img_izq, img_der):
    fig, (ax_izq, ax_der) = plt.subplots(1, 2, figsize=(12, 6))
    ax_izq.imshow(img_izq)
    ax_izq.set_title("Haz clic en la imagen izquierda")
    ax_der.imshow(img_der)
...
def interactive_epipolar_view_E(img_l, img_d, E, T1, T2, K):
    """
    Interactive function: user clicks a point in the left image,
    and the corresponding epipolar line is drawn in the right image.
    """
    ...
    while True:
        ...
        if x < w:
            ...
            axs[0].scatter(x_img, y_img, color='red', s=60)
            p = np.array([x_img, y_img, 1]) 
            l = E @ p  # Epipolar line in right image
            draw_line(axs[1], l, img_d.shape, color='g')
            axs[1].set_title('Right Image (epipolar line shown)')
...
def interactive_epipolar_from_left(img_l, img_d, E, K): #Hecho por Jaime
    """
    Click en la imagen izquierda para mostrar la línea epipolar correspondiente
    en la imagen derecha usando la matriz esencial E y la calibración K.
    """
    def draw_epipolar_line(ax, l, shape, color='m'): 
        a, b, c = l
        h, w = shape[:2]
        ...
        if abs(b) > abs(a):
            y_vals = np.array([0, h])
            x_vals = -(b * y_vals + c) / a if abs(a) > 1e-6 else np.full_like(y_vals, -c / b)
        else:
            x_vals = np.array([0, w])
            y_vals = -(a * x_vals + c) / b if abs(b) > 1e-6 else np.full_like(x_vals, -c / a)
    ...
    ax_l.imshow(img_l_rgb)
    ax_l.set_title('Left Image (click to get epipolar line)')
    ax_r.imshow(img_d_rgb)
    ax_r.set_title('Right Image (epipolar line shown)')
    plt.tight_layout()

    print("Click puntos en la imagen izquierda. Cierra la ventana para terminar.")
    while True:
        pts = plt.ginput(1, timeout=0)
        if not pts:
            break
        x, y = pts[0]
        ax_l.scatter(x, y, color='red', s=60)
        ...
        draw_epipolar_line(ax_r, l_pix, img_d.shape)
        plt.draw()

    plt.close(fig)
    print("Cerrada la visualización interactiva.")

class EpipolarViewer: #Hecho por Jaime
    def __init__(self, img_l, img_d, E, K):
        self.img_l = self._fix_image(img_l)
        self.img_d = self._fix_image(img_d)
        self.E = E
        self.K = K
        self.fig, self.axs = plt.subplots(1, 2, figsize=(12, 6))
        self.axs[0].imshow(self.img_l)
        self.axs[1].imshow(self.img_d)
        self.axs[0].set_title("Imagen Izquierda")
        self.axs[1].set_title("Imagen Derecha")

        # Fijar aspecto y límites para que no se deforme la imagen
        self.axs[0].set_aspect('equal')
        self.axs[1].set_aspect('equal')
        self.axs[0].set_xlim([0, self.img_l.shape[1]])
        self.axs[0].set_ylim([self.img_l.shape[0], 0])
        self.axs[1].set_xlim([0, self.img_d.shape[1]])
        self.axs[1].set_ylim([self.img_d.shape[0], 0])
    ...
    def onclick(self, event):
        ...
        x, y = event.xdata, event.ydata
        self.axs[0].plot(x, y, 'ro')

        p = np.array([x, y, 1.0])
        l = self.E @ p  # l = [a, b, c]
        a, b, c = l

        h, w, _ = self.img_d.shape
        try:
            if abs(b) > 1e-8:
                y0 = -(a * 0 + c) / b
                y1 = -(a * (w - 1) + c) / b
                ...
                x_fixed = -c / a
                x_vals = [x_fixed, x_fixed]
                y_vals = [0, h - 1]

            self.axs[1].plot(x_vals, y_vals, 'r-')
        except Exception as e:
            print(f"[Error en línea epipolar]: {e}")

        self.fig.canvas.draw()

def interactive_epipolar_view_E2(img_l, img_d, E, K):
    """
...
def main():
    ...
    img_l = cv2.imread('cones/im_2.png', cv2.IMREAD_COLOR)
    img_d = cv2.imread('cones/im_6.png', cv2.IMREAD_COLOR)
    flag = True
    ...
    while(flag):
        ...
        def caso_3():
            nonlocal img_l, img_d, puntos_clave_l, puntos_clave_d, robust_sift_ran, K
            ...
            h, w = img_l.shape[:2]
            print("Centro esperado:", w / 2, h / 2)
            print("Centro de K:", K[0, 2], K[1, 2])
            K[0, 2] = w / 2
            K[1, 2] = h / 2
            print("fx = ", K[0,0]) #1986
            print("fy = ", K[1,1])
        ...
        def caso_6():
            nonlocal F, puntos, robust_sift_ran, ransac_ran, puntos_l, puntos_d, T1, T2
            ...
            r = 100000
            F, puntos, Tr1, Tr2 = ransac(puntos_clave_l, puntos_clave_d, r, RANSAC_THRESHOLD)
            T1 = Tr1
            T2 = Tr2
            ransac_ran = True
        ...
        def caso_9():
            nonlocal img_l, img_d, E, K, T1, T2, K
            if E is None or K is None:
                print("You must compute E (option 7) and have K available.")
                return "Aborted: E or K not available."
            #interactive_epipolar_view_E(img_l, img_d, E, T1, T2, K)
            #viewer = EpipolarViewer(img_l, img_d, E, K)
            mostrar_linea_epipolar(E, img_l, img_d)
        ...
        def caso_20():
            ...
            if choice in ['2', '3']:
                ...
                Images = [cv2.imread(file) for file in glob.glob(image_path + '/*.jpeg')]
